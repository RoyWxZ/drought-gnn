...
def normalize_data(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        dataset_without_datetime = dataset.drop(["datetime", "row index"], axis=1) #drop datetime column for normalization since it is not a numerical feature
        ...
        dataset = pd.concat([dataset[['Month','Lat', 'Lon', "datetime", "row index"]], dataset_normalized], axis=1)
